Building a website is easy: just some HTML styled with CSS and with some JavaScript handlers for the user interaction. But what about a desktop application? I have never liked to build desktop applications because they generally depends on the platform they run.
...
## Installation
Currently there are binaries only for Linux (tested on Ubuntu 14.10). To install the latest BAT version, run the following commands:
